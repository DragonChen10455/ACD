...
func CheckMarshalType(mt MarshalType) error {
	if mt < 0 || mt > 19 {
		return fmt.Errorf("MarshalType should be in range [0..19]; got %d", mt)
	}
	return nil
}
...
func MarshalValues(dst []byte, values []int64, precisionBits uint8) (result []byte, mt MarshalType, firstValue int64) {
	...
	return marshalInt64ACD(dst, values, precisionBits)
}
...
func unmarshalInt64Array(dst []int64, src []byte, mt MarshalType, firstValue int64, itemsCount int) ([]int64, error) {
	...
	case MarshalTypeZSTDDeltaXor:
		...
		return dst, nil
	case MarshalTypeDeltaLZ4:
		dst, err = unmarshalInt64DeltaLZ4(dst, src, firstValue, itemsCount)
		if err != nil {
			return nil, fmt.Errorf("cannot unmarshal value for segment self-adaption: %w", err)
		}
		return dst, nil
	case MarshalTypeDelta2LZ4:
		dst, err = unmarshalInt64Delta2LZ4(dst, src, firstValue, itemsCount)
		if err != nil {
			return nil, fmt.Errorf("cannot unmarshal value for segment self-adaption: %w", err)
		}
		return dst, nil
	case MarshalTypeDeltaSnappy:
		dst, err = unmarshalInt64DeltaSnappy(dst, src, firstValue, itemsCount)
		if err != nil {
			return nil, fmt.Errorf("cannot unmarshal value for segment self-adaption: %w", err)
		}
		return dst, nil
	case MarshalTypeDelta2Snappy:
		dst, err = unmarshalInt64Delta2Snappy(dst, src, firstValue, itemsCount)
		if err != nil {
			return nil, fmt.Errorf("cannot unmarshal value for segment self-adaption: %w", err)
		}
		return dst, nil
	case MarshalTypeGorillaZ:
		dst, err = unmarshalInt64Gorilla(dst, src, MarshalTypeGorillaZ, firstValue, itemsCount)
		if err != nil {
			return nil, fmt.Errorf("cannot unmarshal value for segment self-adaption: %w", err)
		}
		return dst, nil
	case MarshalTypeACD:
		dst, err = unmarshalInt64ACD(dst, src, firstValue, itemsCount)
		if err != nil {
			return nil, fmt.Errorf("cannot unmarshal value for segment self-adaption2: %w", err)
		}
		return dst, nil
	default:
